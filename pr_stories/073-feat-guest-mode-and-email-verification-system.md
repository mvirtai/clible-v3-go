...
### 3. Backend: Optional Auth Middleware
...
### 5. Frontend: Verify Email Page & Auth Flow
...
- **Statement coverage (total):** 74.6%
- **New test cases:** 16 auth service tests + 3 mailer tests covering registration, login rejection, OTP verification, token verification, expiration, resend, and JWT lifecycle
- **Security:** Cryptographic OTP/token generation, bcrypt cost 12, JWT with HMAC-SHA256 + issuer/audience validation
...
| `frontend/src/components/layout/AppHeader.tsx` | Guest-aware header rendering |
| `frontend/src/components/layout/WorkspaceSidebar.tsx` | Guest-aware sidebar |
| `frontend/src/utils/i18n.ts` | Verification-related translations (FI/EN) |
...
- **Total coverage:** 74.6% of statements
- **Services coverage:** 80.0%
- **API coverage:** 75.7%
- **DB coverage:** 72.0%
- **Config coverage:** 97.1%
- **All packages:** PASS

```text
ok  github.com/mvirtai/clible-v3-go/internal/api        3.123s  coverage: 75.7%
ok  github.com/mvirtai/clible-v3-go/internal/config      0.015s  coverage: 97.1%
ok  github.com/mvirtai/clible-v3-go/internal/ctxkeys     0.008s  coverage: 100.0%
ok  github.com/mvirtai/clible-v3-go/internal/db          0.649s  coverage: 72.0%
ok  github.com/mvirtai/clible-v3-go/internal/dsl         0.022s  coverage: 79.3%
ok  github.com/mvirtai/clible-v3-go/internal/middleware   0.066s  coverage: 83.9%
ok  github.com/mvirtai/clible-v3-go/internal/parsers      0.020s  coverage: 93.8%
ok  github.com/mvirtai/clible-v3-go/internal/services     3.300s  coverage: 80.0%
ok  github.com/mvirtai/clible-v3-go/internal/version      0.007s  coverage: 100.0%
```
...
2. **Migration idempotency:** Confirmed `014_email_verification.sql` applies cleanly on fresh SQLite `:memory:` databases during test initialization.
3. **React Compiler compliance:** All frontend components converted from `React.FC` to plain function declarations — no `useEffect` or `useRef` introduced.
