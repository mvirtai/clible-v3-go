...
1. **Functional Pipeline Syntax**: Standardizes all transformations and modifiers on the `=>` pipe operator using functional parenthesis notation: `in(KR92)`, `vs(KR92, KR38)`, `refs(3)`, `themes(5)`, `suggest(3)`, `count()`, and `limit(5)`.
2. **Bilingual Smart Scopes**: Equips search queries and count aggregations with natural Finnish and English biblical groupings (`@evankeliumit` / `@gospels`, `@toora` / `@torah`, `@kirjeet` / `@epistles`, `@viisaus` / `@wisdom`, `@profeetat` / `@prophets`, `@historia` / `@history`, `@VT` / `@OT`, `@UT` / `@NT`).
...
4. **Clean Markdown Embedding**: Replaces double-bracket embeds with clean single-bracket notation (`![@Joh 3:16 => in(KR92)]`) and enhances live IntelliSense autocomplete in the editor.
...
sequenceDiagram
    ...
    User->>MC: Types `![search("armo") => @evankeliumit => in(KR92) => count()]`
...
```mermaid
flowchart TD
    Raw["Raw Input Directive (e.g., ! @Joh 3:16 => vs(KR92, KR38))"] --> Lexer["Lexer (Token Stream)"]
    Lexer --> Parser["Recursive Descent Parser"]
    Parser --> AST["AST: PipeNode / ActionNode(args)"]
    
    AST --> Dispatcher{"AST Node Dispatcher"}
    
    Dispatcher -->|"in(TID)"| TransExec["executeVerseRef(transID)"]
    ...
    Dispatcher -->|"@scope"| ScopeExec["resolveSearchScope -> multi-book query"]
...
* **Test Coverage:** Backend total statement coverage maintained at **78.9%** across all packages, with full DSL AST, Lexer, Parser, and Executor unit test suites passing.
* **Frontend Test Suite:** 100% passing across all 25 test suites (**135 tests**).
...
| `backend/internal/dsl/parser.go` | Implemented recursive descent parser for functional arguments (`in`, `vs`, `refs`, `themes`, `suggest`, `count`, `limit`, `@scope`). |
| `backend/internal/dsl/executor.go` | Added `resolveSearchScope` with bilingual groups, extended `ExecutionContext` with finder hooks, and implemented functional execution. |
...
| `docs/guide/isla-guide.md` | Updated complete user documentation with functional pipeline syntax, smart scope examples, and single-bracket embed guidelines. |
...
| `frontend/src/components/notebook/cells/MarkdownCell.tsx` | Updated `preprocessContent` to parse single bracket `![...]` embeds and functional directives. |
...
| `frontend/src/components/notebook/isla/islaIntellisense.ts` | Updated autocomplete snippets and pipeline suggestions to functional forms. |
...
| `frontend/src/utils/islaClassifier.ts` & `test.ts` | Cleaned classifier helper to align with unified markdown cells. |
