...
func (r *NotebookRepository) Create(ctx context.Context, nb *models.Notebook) error {
	...
	_, err := r.db.ExecContext(ctx, query, nb.ID, nb.Title, nb.UserID, scopeID, nb.CreatedAt, nb.UpdatedAt)
	if err != nil {
...
func (r *NotebookRepository) Update(ctx context.Context, nb *models.Notebook) error {
	query := `
		UPDATE notebooks
		SET title = $1, scope_id = $2, update_at = $3
	...
	_, err := r.db.ExecContext(ctx, query, nb.Title, scopeID, nb.UpdatedAt, nb.ID, nb.UserID)
	if err != nil {
...
func (r *NotebookRepository) SaveCells(ctx context.Context, notebookID string, cells []models.Cell) error {
	...
	query := `
		INSERT INTO notebook_cells (id, notebook_id, type, content, result_json, position, created_at, updated_at)
...
func (r *NotebookRepository) GetCells(ctx context.Context, notebookID string) ([]models.Cell, error) {
	query := `
		SELECT id, notebook_id, type, content, result_json, position, created_at, updated_at
		FROM notebook_cells
		WHERE notebook_id = $1
