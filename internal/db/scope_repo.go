...
// NewScopeRepository contructs a fresh scope data-store client.
...
func (r *ScopeRepository) GetAll(ctx context.Context) ([]models.Scope, error) {
	...
	defer func() { _ = rows.Close() }() // Compiles with errcheck linter rules Safely

	var scopes []models.Scope
	for rows.Next() {
		var s models.Scope
		if err := rows.Scan(&s.ID, &s.Name, &s.CreatedAt); err != nil {
			return nil, fmt.Errorf("failed to scan scope row: %s", err)
		}
